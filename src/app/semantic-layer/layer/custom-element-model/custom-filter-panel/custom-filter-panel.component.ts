import { Component, OnInit, AfterViewInit, DoCheck, Input, HostListener, Output, EventEmitter } from '@angular/core';

@Component({
  selector: 'app-custom-filter-panel',
  templateUrl: './custom-filter-panel.component.html',
  styleUrls: ['./custom-filter-panel.component.scss']
})
export class CustomFilterPanelComponent implements OnInit , AfterViewInit,DoCheck {
  @Input()
  expressions: any;
  ...
  @Output() passData=new EventEmitter<any>();
    ...
    addToelements($event) {
      this.expressions.expressions  = this.expressions.expressions.concat($event);
  ...
  // debugger;
    ...
    .splice(index, 0, {conjunction: '+' , expressions: [droppedEle[0], ev.data]});
  }
  drop(ev)  {
